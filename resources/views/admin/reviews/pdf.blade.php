...
<head>
    ...
    <style>
        ...
        /* 2. COLORS (Kostarae Palette) */
        .text-indigo { color: #1e3a8a; } /* Biru Tua/Indigo */
        ...
        /* 5. SUMMARY CARDS (GRID 2x2 via TABLE) */
        .summary-table { width: 100%; border-collapse: separate; border-spacing: 10px 10px; margin-left: -10px; margin-right: -10px; }
        .card {
            border: 1px solid #cbd5e1;
            padding: 15px;
            border-radius: 8px; /* Rounded corner (support terbatas di dompdf lama, tapi aman di baru) */
        ...
        .card-sub { font-size: 8pt; color: #16a34a; margin-top: 5px; } /* Hijau untuk status positif */

        /* 6. STATUS TABLE (Small) */
...
<body>
    ...
    <table class="summary-table">
        <tr>
            <td class="card">
                <div class="card-label">Total Pencari Kost</div>
                <div class="card-value">{{ number_format($stats['total_user']) }}</div>
                <div class="card-sub">👤 Pengguna Terdaftar</div>
            </td>
            <td class="card">
                <div class="card-label">Unit Kost Aktif</div>
                <div class="card-value">{{ number_format($stats['total_kost']) }}</div>
                <div class="card-sub">🏠 Siap Huni</div>
            </td>
        </tr>
        <tr>
            <td class="card">
                <div class="card-label">Perlu Review</div>
                <div class="card-value" style="color: #d97706;">{{ number_format($stats['pending_kost']) }}</div>
                <div class="card-sub" style="color: #d97706;">⏳ Menunggu Verifikasi</div>
            </td>
            <td class="card">
                <div class="card-label">Kualitas Ekosistem</div>
                <div class="card-value">{{ number_format($stats['avg_rating'], 1) }} <span style="font-size: 14pt; color: #ccc;">/ 5.0</span></div>
                <div class="card-sub">⭐ Rata-rata Rating</div>
            </td>
        </tr>
    </table>
        ...
        <tr>
            ...
            <td width="45%" style="vertical-align: top;">
                <table class="status-table">
                    ...
                    <tbody>
                        <tr>
                            <td><span style="color: #15803d; font-weight:bold;">●</span> Aktif</td>
                            <td class="text-right"><strong>{{ $stats['total_kost'] }}</strong></td>
                        </tr>
                        <tr>
                            <td><span style="color: #d97706; font-weight:bold;">●</span> Pending (Review)</td>
                            <td class="text-right">{{ $stats['pending_kost'] }}</td>
                        </tr>
                        {{-- Contoh data statis untuk rejected jika belum ada di controller --}}
                        <tr>
                            <td><span style="color: #dc2626; font-weight:bold;">●</span> Ditolak/Non-Aktif</td>
                            <td class="text-right">0</td>
                        </tr>
                        <tr style="background-color: #f1f5f9; font-weight: bold;">
                            <td style="border-top: 1px solid #ccc;">TOTAL UNIT</td>
                            <td class="text-right" style="border-top: 1px solid #ccc;">{{ $stats['total_kost'] + $stats['pending_kost'] }}</td>
                        </tr>
                    </tbody>
                </table>
            </td>
    ...
    <table class="data-table">
        <thead>
            <tr>
                ...
                <th width="20%" class="text-center">Rating Rata-rata</th>
            </tr>
        </thead>
        <tbody>
    ...
    <div class="conclusion-box">
        ...
        Kualitas rata-rata mitra kost berada di angka {{ number_format($stats['avg_rating'], 1) }}/5.0, yang menunjukkan kepuasan pengguna yang baik. 
    ...
    <table class="signature-table">
        <tr>
            ...
            <td width="20%"></td> <td class="sign-col">
